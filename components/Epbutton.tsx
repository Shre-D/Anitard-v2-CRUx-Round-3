...
import { reception } from "./Anilist/getAnime";
...
function Epbutton({ value, id, info, chooseEpisode, getEpisodeNo }: Props) {
  ...
  const getEpisode = async () => {
    const EpisodeLink = await (
      await fetch(
        `https://api.consumet.org/anime/gogoanime/watch/${title}-episode-${value}`
      )
    ).json();
    const EpisodeLink2 = await (
      await fetch(
        `https://api.consumet.org/anime/gogoanime/watch/${fallbackTitle}-episode-${value}`
      )
    ).json();
